...
// 3. Fetch live now
// GET /api/station/:stationId/schedule/live citeturn2view0
async function fetchLiveNow() {
  try {
    ...
    if (result.status === "schedule") {
      const ev = result.content;
      nowDjEl.textContent = ev.title;
      nowDjEl.style.display = "block";
...
// GET /api/station/:stationId/schedule?startDate=<iso>&endDate=<iso> citeturn2view0
...
// 6. Pop‐out player and recurring initialization
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (popOutBtn) {
    popOutBtn.addEventListener("click", () => {
      ...
      pop.document.write(\`
        <!DOCTYPE html>
        <html lang="en"><head><title>CCR Player</title></head>
        <body style="margin:0">
